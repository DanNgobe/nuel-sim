...
def main(episodes=2000):
    ...
    for episode in range(episodes):
        ...
        while not done:
            ...
            prev_observations = game_manager.get_prev_observations()
            ...
            if 'history' in info and info['history']:
                for shooter, target, hit in info['history']:
                    if shooter and target and shooter.id in prev_observations:
                        shooter_obs = prev_observations[shooter.id]
                        reward = rewards.get(shooter.id, 0.0)
                        
                        # Get next observation
                        next_obs = observations.get(shooter.id, shooter_obs)
                        
                        # Get action (target index)
                        targets = config.OBSERVATION_MODEL.get_targets(shooter, game_manager.current_game.players)
