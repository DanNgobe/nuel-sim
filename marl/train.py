...
from marl.settings import MEMORY_SIZE, TARGET_UPDATE_FREQ, OBSERVATION_MODEL
import config
import torch
import random
import argparse
...
def main(episodes=2000):
    ...
    for episode in range(episodes):
        ...
        while not done:
            prev_obs = {player.name: [player.alive, OBSERVATION_MODEL.create_observation(player, players)] for player in players} # store observations for all players
            game.run_turn()
            done = game.is_over()
            ...
            for shooter, target, hit in last_history:
                if shooter and target:
                    ...
                    next_obs = OBSERVATION_MODEL.create_observation(shooter, players)
